...
def ttl_create(client, key, value, ttl_value):
    cache = caches.get(ttl_value, TTLCache(maxsize=1024, ttl=ttl_value))
    cache[key] = value
    caches[ttl_value] = cache
    return client


def time_to_live_check(client, key):
    if not caches: return 'cache_empty'
    try:
...
def check_key_exist(dict, key, client_name):
    if key in dict:
        ttl_status = "no_ttl"
        if dict[key][1] == 1 : 
            ttl_status = time_to_live_check(client_name, key)
